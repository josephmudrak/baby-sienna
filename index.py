...
conversation_embeddings = pd.read_csv(
    "csvs/conversation_embeddings_psa1.csv"
).values

conversation_csv = "csvs/updated_conversations.csv"
...
def add_conversation(conv, csv):
    df = pd.read_csv(csv)
    last_row = df.iloc[-1]
    id = int(last_row["ID"]) + 1
    message = str(date.today().strftime("%B %d, %Y"))
    ...
    for i in conv:
        ...
        if data["role"] == "user":
            message += f" Liam: {data["content"]}"
        else:
            message += f" {data["content"]}"

    new_row = pd.DataFrame({"ID": [id], "Conversation": [message]})
    new_row.to_csv(csv, mode="a", header=False, index=False)
